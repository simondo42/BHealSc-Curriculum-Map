...
# mat <- matrix(ncol = 17, nrow = nrow(filter(CMHC_nodes, type == TRUE)))
# mat <- as.data.frame(mat) %>%
#   as_tibble() %>%
#   mutate(across(everything(), as.character)) %>%
#   bind_cols(select(filter(CMHC_nodes, type == TRUE), Node))
# 
# names(mat) <- c(letters[1:17], "Paper")
# 
# CMHC <- CMHC %>%
#   mutate(Letter = str_sub(BHealSc_Outcome, 1, 1))
# 
# for (i in 1:nrow(CMHC)) {
#   mat[mat$Paper == CMHC$Paper[i], str_which(letters, CMHC$Letter[i])] <- " "
# }
# 
# mat <- relocate(mat, Paper, .before = a)
# 
